...
export class OfertasService {
  ...
  */
  ...
  getOfertaByItemSearch(search): Observable<any> {
    const urlApi = '/api/oferta/buscador-by-titulo?start=0&length=10&search=' + search;
    return this.httpClient.get(urlApi);
  }
  ...
  updateOferta(oferta: OfertaFreelancer): Observable<any> {
    const urlApi = '/api/oferta/' + oferta.id;
    let headers =  new HttpHeaders();
    headers = headers.set('Authorization', 'Bearer ' + this.authService.getToken());
